...
BOOL CreateRegKey(LPWSTR wStrRegKeyName, LPWSTR wStrRegKeyValue) { 
	...
	DWORD bufferIncrement = 4096;
	//PPERF_DATA_BLOCK buffer = (PPERF_DATA_BLOCK)malloc(bufferSize);
	...
	/*while (lRegStatus == ERROR_MORE_DATA) {
		bufferSize += bufferIncrement;
		buffer = (PPERF_DATA_BLOCK) realloc(buffer,bufferSize);
		lRegStatus = RegQueryValueExW(hKey, wStrRegKeyName, NULL, NULL, (LPBYTE)buffer, &dwSizeofRegBuffer);
	}
	if (lRegStatus != ERROR_SUCCESS) {
		return bSuccess;
	}*/

	if (lRegStatus==ERROR_FILE_NOT_FOUND) {
