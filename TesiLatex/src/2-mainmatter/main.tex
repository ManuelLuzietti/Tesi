...
In un Fast-flux network (\Cref{fig:fast-flux}) il servizio Domain Name System (DNS) di risoluzione degli \textit{hostname} viene usato come \textit{multiplexer} e \textit{load balancer}. La macchina infetta non comunica direttamente col server usando il suo indirizzo IP ma punterà a risolvere un hostname attraverso risoluzione DNS. Al record DNS in cui vi è registrato l'hostname da risolvere saranno associati molteplici indirizzi IP appartenenti a più bot della Botnet. Così facendo si sfrutta le capacità native di \textit{load balancing} dei server DNS per ciclare sugli indirizzi IP assegnati. I bot così associati a questo hostname sono gli unici a conoscenza dell'indirizzo IP del C\&C server e quindi sono gli unici in grado di comunicare con esso. 
...
Spesso utilizzano funzionalità di api hooking (\Cref{apihooking}) per intercettare le chiamate a funzioni di libreria del sistema al fine di nascondere l'esecuzione di file malevoli o attività di essi.
...
\begin{figure}[hbtp]
    \centering
    \includegraphics[width=\textwidth]{infrastruttura1.png}
...
Esistono due tipi di hypervisor (\Cref{fig:hypervisors}):
\begin{description}
...
\begin{figure}[hbtp]
    \centering
    \includegraphics[width=8cm]{hypervisors.png}
...
In fine prima di selezionare un modulo Firewall tra le opzioni disponibili offerte da  ClearOS, sono stati studiati Netfilter (\ref{netfilter}), Iptables (\ref{iptables}), Nftables (\ref{nftables}) e Firewalld (\ref{firewalld}). Una volta comprese queste tecnologie è stata scelta una soluzione (modulo) basata su Iptables (\Cref{fig:clearos}).
...
\begin{figure}[hbtp]
    \centering
    \includegraphics[width=\textwidth]{clearos.png}
...
\begin{figure}[hbtp]
    \centering
    \includegraphics[width=8cm]{docker.png}
...
RUN make /app
\end{lstlisting}
Quando viene eseguita la \textit{build} dell'immagine, ogni operazione all'interno del Dockerfile costituisce un livello che viene posta sopra al livello dell'operazione precedente \Cref{fig:dockerlayers}. Ogni livello contiene solo le differenze dal livello precedente. In questo modo se la build dovesse fallire, al tentativo successivo riprenderebbe dal livello in cui si era interrotto. Questo meccanismo permette anche di accelerare i tempi in caso di \textit{rebuild} in seguito a piccoli cambiamenti nel Dockerfile, in quanto i livelli verranno ricostruiti dal primo cambiamento a seguire.
\begin{figure}[hbtp]
    \centering
...
Entrando nel dettaglio (\Cref{fig:dockernetwork}), il network 'Bridge' è un bridge di rete, chiamato di default \textit{docker0}, creato quando viene inizializzato il servizio Docker. Ogni container appartiene ad un namespace di rete, per fare in modo che abbia una copia dello stack di rete dell'host e che sia isolato.
...
\begin{figure}[hbtp]
    \centering
    \includegraphics[width=10cm]{dockernetwork.png}
...
Approcci tradizionali di SIDS sfruttano i \textit{log}\footnote{documentazione inerente un determinato evento che ha avuto esito} di una macchina host, generati dal sistema o applicazioni software, e l'analisi del traffico di rete per scovare azioni, comandi, pattern riconducibili a malware noti.
...
\begin{figure}[hbtp]
    \centering
    \includegraphics[width=8cm]{tapvsspan.png}
...
\begin{description}
    ...
    \item [Kibana] Interfaccia GUI (\Cref{fig:kibana}) che permette di visualizzare i dati memorizzati in Elastic Search e di navigare l'Elastic Stack. Permette di visualizzare in tempo reale flussi di dati di notevoli dimensioni e di aumentarne la comprensione attraverso grafici.
\end{description}

\begin{figure}[hbtp]
    \centering
    \includegraphics[width=0.7\textwidth]{kibana.png}
    \caption{Kibana.}
    \label{fig:kibana}
\end{figure}
...
\begin{figure}[hbtp]
    ...
    \begin{subfigure}{0.45\textwidth}
        \centering
        \includegraphics[width=\textwidth]{securityonionconsole1.png}
        \caption{Dashboard.}
        \label{fig:securityonionconsole1}
    \end{subfigure}
    \begin{subfigure}{0.45\textwidth}
        \centering
        \includegraphics[width=\textwidth]{securityonionconsole2.png}
        \caption{Alerts.}
...
\begin{figure}[hbtp]
    \centering
    \includegraphics[width=10cm]{securityonionarchitecture.png}
...
\begin{figure}[hbtp]
    \centering
    \includegraphics[width=\textwidth]{deploysecurityonion.png}
...
\begin{figure}[hbtp]
    \centering
    \includegraphics[width=0.8\textwidth]{ossim.png}
...
\begin{figure}[hbtp]
    \centering 
    \includegraphics[width=0.8\textwidth]{deployossim.png}
...
Alla ricezione di un task,  esegue il parsing e ne esegue comando (\Cref{fig:byobcommandexecution}), cui  risultato viene incapsulato e inoltrato al server.
...
\begin{figure}[hbtp]
    \centering
    \includegraphics[width=\textwidth]{byobcommandexecution.png}
...
\begin{figure}[hbtp]
    ...
    \begin{subfigure}{0.45\textwidth}
        \centering
        \includegraphics[width=\textwidth]{wazuh1.png}
        \caption{visualizzazione eventi.}
        \label{fig:wazuh1}
    \end{subfigure}
    \begin{subfigure}{0.45\textwidth}
        \centering
        \includegraphics[width=\textwidth]{wazuh2.png}
        \caption{Dashboard.}
        \label{fig:wazuh2}
    \end{subfigure}
    \begin{subfigure}{0.45\textwidth}
        \centering
        \includegraphics[width=\textwidth]{wazuh3.png}
        \caption{schermata di gestione degli agent.}
        \label{fig:wazuh3}
    \end{subfigure}
    \begin{subfigure}{0.45\textwidth}
        \centering
        \includegraphics[width=\textwidth]{wazuh4.png}
...
\begin{figure}[hbtp]
    \centering
    \includegraphics[width=0.8\textwidth]{deploywazuh.png}
...
Entrando nei dettagli del loop, invia periodicamente un comando di Poll  a cui master risponde all'occorrenza con il prossimo comando da eseguire. Quando il comando viene ricevuto, ne esegue il parsing e nel caso sia un comando riconosciuto esegue l'handler ad esso associato (\Cref{fig:uboatcommandexecution}). Infine attende una quantità di tempo preimpostata prima di ripetere il ciclo e quindi di rieffettuare il polling verso il server.
...
\begin{figure}[hbtp]
    \centering
    \includegraphics[width=0.8\textwidth]{uboatcommandexecution.png}
...
\input{src/2-mainmatter/capitolo7}


\input{src/2-mainmatter/conclusioni}
