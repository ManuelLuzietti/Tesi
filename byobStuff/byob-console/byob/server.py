...
def main():
    ...
    if len(site_packages):
        ...
        sys.exit(0)
    globals()['packages']  = "/usr/local/lib/python3.6/dist-packages"
    util.log(globals()['packages'])
    ...
    #util.log('{} core/handler.py {}'.format(sys.executable, int(options.port + 3)))
    ...
    def run(self):
        ...
        while True:
            if self._active.wait():
                ...
                util.log(['task received in run',task])
